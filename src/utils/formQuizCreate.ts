...
export function updateAnswerValue(
  ...
  return newQuestions.map((question, index) => {
    if (indexQuestion === index) {
      return {
        ...question,
        answers: question.answers.map((answer, answersIdex) => {
          if (indexAnswer === answersIdex) {
            return {
              ...answer,
              answer: newValue,
            };
...
export function updateRadioBtn(
  newQuestions:Question[],
  indexQuestion: number,
  indexAnswer: number,
) {
...
export function updateAnswerError(
  errorsNewQuiz: IerrorFormNewQuiz,
  indexQuestion: number,
  indexAnswer: number,
) {
  return {
    ...
    questions: errorsNewQuiz.questions.map((questionError, index) => {
      if (indexQuestion === index) {
        return {
          ...questionError,
          answers: questionError.answers.map((answerError, answersIdex) => {
            if (indexAnswer === answersIdex) {
              return {
                ...answerError,
                answer: '',
